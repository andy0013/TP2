...
void ParserSolicitudUsuario::identificarInformacionIngresadaStdin(char *input){
	int i = 0;
...
SplitApplyCombineImpl* ParserSolicitudUsuario::crearSplitApplyCombineConInformacionIngresada(char *argv[]) {
	SplitApplyCombineImpl* ejecutorSplitApplyCombine = new SplitApplyCombineImpl(
			std::stoi(this->filaInicio), std::stoi(this->filaFin), this->operacion,
			std::stoi(this->nroParticiones));
	ejecutorSplitApplyCombine->cargarDatosParaResolverOperaciones(argv, std::stoi(this->columnaPorUsar));
	return ejecutorSplitApplyCombine;
}


int ParserSolicitudUsuario::obtenerValor(std::string* value, char* input, int inicio){
	int i;
